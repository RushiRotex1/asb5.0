  ...
  try {
    ...
    if (e.postData && e.postData.contents) {
      try {
        ...
        return createCORSResponse(JSON.stringify({
          availableActions: ['getLabels', 'getValues', 'getOptions', 'updateValue', 'getResults', 'getFinalResults', 'getExtendedLabels', 'testConnection', 'clearAllValues', 'getApplicationTypes', 'getCustomRange', 'autoSelectSingleOptions'],
...
function getDropdownLabels() {
  try {
    ...
    // Add extended question labels
    const extendedCells = ['A106', 'A135', 'A139', 'A142', 'A147', 'A151', 'A154', 'A164', 'A167', 'A172', 'A176', 'A180', 'A185', 'A188', 'A191', 'A194'];
    
    for (let i = 0; i < extendedCells.length; i++) {
      const cell = extendedCells[i];
      try {
        const labelValue = sheet.getRange(cell).getValue();
        labels[cell] = labelValue ? labelValue.toString().trim() : '';
        console.log(`Extended label ${cell}:`, labels[cell]);
      } catch (cellError) {
...
function getExtendedLabels() {
  try {
    console.log('Fetching extended labels from A column cells...');
    const spreadsheet = getSpreadsheet();
    const sheet = spreadsheet.getSheetByName(SHEET_NAME);
    ...
    // Define the A column cells that correspond to extended questions
    const extendedCells = ['A106', 'A135', 'A139', 'A142', 'A147', 'A151', 'A154', 'A164', 'A167', 'A172', 'A176', 'A180', 'A185', 'A188', 'A191', 'A194'];
    
    const labels = {};
    
    // Get all labels in one batch operation for better performance
    for (let i = 0; i < extendedCells.length; i++) {
      const cell = extendedCells[i];
      try {
        const cellValue = sheet.getRange(cell).getValue();
        labels[cell] = cellValue ? cellValue.toString().trim() : '';
        console.log(`Extended label ${cell}: "${labels[cell]}"`);
      } catch (cellError) {
        console.error(`Error getting extended label for ${cell}:`, cellError);
        labels[cell] = '';
    ...
    console.log(`Found ${Object.keys(labels).length} extended labels`);
    return labels;
  } catch (error) {
    console.error('Error getting extended labels:', error);
    // Return empty labels object on error
    const emptyLabels = {};
    const extendedCells = ['A106', 'A135', 'A139', 'A142', 'A147', 'A151', 'A154', 'A164', 'A167', 'A172', 'A176', 'A180', 'A185', 'A188', 'A191', 'A194'];
    ...
    // Add fallback extended labels
    const extendedFallbacks = {
      'A106': 'Media Selection',
      'A135': 'Body Material Selection', 
      'A139': 'Secondary Material Selection',
      'A142': 'Manual Actuation Type',
      'A147': 'Actuation Selection',
      'A151': 'Connection Type',
      'A154': 'Port Size',
      'A164': 'Electrical Connection',
      'A167': 'Voltage Selection',
      'A172': 'Certification',
      'A176': 'Special Features',
      'A180': 'Temperature Range',
      'A185': 'Pressure Rating',
      'A188': 'Flow Rate',
      'A191': 'Response Time',
      'A194': 'Additional Options'
    };
    
    Object.assign(fallbackLabels, extendedFallbacks);
    return emptyLabels;
  }
}
    return [];
